...
<script>
import { fetchVisdeviceList, addVisDevice, deleteVisDevice, editVisDevice, changeServerUrl } from '@/api/device'
import { loadAllVisDevice } from '@/api/personnel'
...
export default {
  ...
  methods: {
    getList(data) {
      ...
      fetchVisdeviceList(data).then(response => {
        ...
        if (valid) {
          ...
          if (isIp) {
            ...
            $.ajax({
              ...
              editVisDevice(this.addDeviceForm).then(res => {
    ...
    handleDelete(deviceCode) {
      deleteVisDevice(deviceCode).then(res => {
        if (res.status === 200) {
          this.$message.success(res.message)
        } else {
          this.$message.error(res.message)
          ...
          if (isIp) {
            ...
            $.ajax({
              ...
              if (res.code === 200) {
                ...
                addVisDevice(this.addDeviceForm).then(res => {
                  this.dialogFormVisible = false
                  if (res.status === 200) {
                    this.$message.success(res.message)
                    this.getList(this.searchForm)
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        (async() => {
          let jsonList = ''
          await fetchVisdeviceList().then(response => { jsonList = response.data.items })
          ...
          excel.export_json_to_excel({
            header: tHeader,
            data,
            filename: 'VisDevice：' + parseTime(new Date())
          })
