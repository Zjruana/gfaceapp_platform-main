...
<script>
import { fetchBlacklistRecordList } from '@/api/record'
...
export default {
  ...
  methods: {
    getList(data) {
      ...
      fetchBlacklistRecordList(data).then(response => {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        (async() => {
          let jsonList = ''
          await fetchBlacklistRecordList().then(response => { jsonList = response.data.items })
          ...
          excel.export_json_to_excel({
            header: tHeader,
            data,
            filename: 'VisRecord：' + parseTime(new Date())
          })
