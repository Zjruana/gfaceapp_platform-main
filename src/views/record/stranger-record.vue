...
<script>
import { fetchStrangertRecordList } from '@/api/record'
...
export default {
  ...
  methods: {
    getList(data) {
      ...
      fetchStrangertRecordList(data).then(response => {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        (async() => {
          let jsonList = ''
          await fetchStrangertRecordList().then(response => { jsonList = response.data.items })
