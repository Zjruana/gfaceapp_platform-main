...
<script>
import { fetchVisitorList, addVisUser, loadAllVisDevice, deleteVisUser, editVisUser } from '@/api/personnel'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      batchPersonnelListBtnDisabled: true,
      searchForm: {
        name: null,
        mobilePhone: null,
        sex: null,
  ...
  methods: {
    getList(data) {
      ...
      fetchVisitorList(data).then(response => {
        this.userImgList = []
    ...
    async handleEdit(info) {
      await loadAllVisDevice().then((res) => {
    ...
    handleDelete(personId) {
      deleteVisUser(personId).then(res => {
        if (res.status === 200) {
    ...
    editUser(form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          ...
          editVisUser(this.addUserForm).then(res => {
    ...
    async clickBatchPersonnelList() {
      await loadAllVisDevice().then(async(r) => {
        ...
        for (const item of this.batchPersonnelList) {
          ...
          await addVisUser(item).then((res) => {
            this.dialogFormVisible = false
    ...
    addUsers(addUserForm) {
      ...
      this.$refs[addUserForm].validate((valid) => {
        if (valid) {
          this.isLoading = true
          addVisUser(this.addUserForm).then(res => {
            this.dialogFormVisible = false
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        (async() => {
          let jsonList = ''
          await fetchVisitorList().then(response => { jsonList = response.data.items })
          const tHeader = ['员工姓名', '手机', '门禁卡号', '身份证', '设备组', '过期时间', '创建时间', '员工照片', '工号', '开门密码', '备注']
          const filterVal = ['name', 'mobilePhone', 'icCard', 'idCardNo', 'deviceGroup', 'validDateEnd', 'validDateStart', 'faceUrl', 'jobNumber', 'openDoorPassword', 'remark']
