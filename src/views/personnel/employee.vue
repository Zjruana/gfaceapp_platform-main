...
<script>
import { fetchEmployeeList, addUser, loadAllDevice, deleteUser, editUser } from '@/api/personnel'
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        (async() => {
          let jsonList = ''
          await fetchEmployeeList().then(response => { jsonList = response.data.items })
